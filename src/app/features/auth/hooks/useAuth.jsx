...
import GsapEntrance from '../ui/components/GsapEntrance';
...
export const useAuth = () => {
  ...
  const createSubmitHandler = (callback) => async (data) => {
    setIsLoading(true);
    try {
      await callback(data);
    } finally {
  ...
  return { isLoading, showPass, showConfirm, togglePass, toggleConfirm, createSubmitHandler,navigate };
};
...
export const SubmitButton = ({ label, isLoading }) => (
  <div data-animate>
    <button type="submit" disabled={isLoading} className="relative w-full rounded-lg py-3 mt-1 text-sm font-semibold tracking-widest uppercase overflow-hidden bg-white text-black hover:bg-zinc-100 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-200 group">
      ...
      <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 -skew-x-12 translate-x-[-100%] group-hover:translate-x-[200%] transition-all duration-700" />
    </button>
  </div>
);
